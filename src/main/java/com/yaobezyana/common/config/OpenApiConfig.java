                ...
                .info(new Info()
                        ...
                        .description("""
                                ...
                                **Завершение:**
                                ...
                                Автоматически создаётся следующий спринт. ИИ генерирует текстовое резюме спринта.

                                ## ИИ-функции (Ollama)

                                Три AI-эндпоинта, работающие через локальную модель (Ollama, без облачных затрат).
                                Если Ollama недоступна — возвращается `503 Service Unavailable`.

                                | Эндпоинт | Описание |
                                |----------|----------|
                                | `GET /api/inbox/{id}/suggest` | Предлагает тип конвертации, заголовок и проект для заметки |
                                | `GET /api/sprints/current/suggest-tasks` | Рекомендует задачи для спринта по каждому проекту |
                                | `PATCH /api/sprints/current/complete` | При завершении спринта авто-генерирует резюме (поле `summary`) |

                                Все предложения ИИ — **ненавязывающие**: финальный выбор остаётся за пользователем.
                                ...
                                ИИ-подсказка перед конвертацией: `GET /api/inbox/{id}/suggest`.
                                ...
                                | `COMPLETED` | Спринт завершён (содержит `summary`) |
